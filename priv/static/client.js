(()=>{function c(){let o;if((o=document.querySelector("[data-phx-main]"))||(o=document.querySelector('[id^="phx-"]')))return o.id;if(o=document.querySelector("[data-phx-root-id]"))return o.getAttribute("data-phx-root-id")}window.getLiveDebuggerURL=function(){let o=document.getElementById("live-debugger-scripts").src.replace("/assets/live_debugger/client.js",""),n=c(),i=n?`transport_pid/${n}`:"";return`${o}/${i}`};document.addEventListener("DOMContentLoaded",function(){let o=getLiveDebuggerURL(),n=`
  ...
  `,i=document.createElement("div");i.innerHTML=n;let e=i.firstElementChild;document.body.appendChild(e);let l=!1,s=t=>{t.button!==0||t.ctrlKey||(t.preventDefault(),posXStart=t.clientX,posYStart=t.clientY,document.addEventListener("mousemove",r),document.addEventListener("mouseup",d),e.style.cursor="grabbing",l=!1)},r=t=>{!t.clientX||!t.clientY||(l=!0,posX=posXStart-t.clientX,posY=posYStart-t.clientY,posXStart=t.clientX,posYStart=t.clientY,e.style.top=`${e.offsetTop-posY}px`,e.style.left=`${e.offsetLeft-posX}px`)},d=()=>{document.removeEventListener("mousemove",r),document.removeEventListener("mouseup",d),e.style.cursor="grab",e.offsetTop<0&&(e.style.top=e.style.bottom),e.offsetTop+e.clientHeight>window.innerHeight&&(e.style.top=""),e.offsetLeft<0&&(e.style.left=e.style.right),e.offsetLeft+e.clientWidth>window.innerWidth&&(e.style.left="")},u=t=>{l&&(t.preventDefault(),l=!1)};window.addEventListener("resize",()=>{e.offsetLeft+e.clientWidth+Number.parseInt(e.style.right)>window.innerWidth&&(e.style.left=""),e.offsetTop+e.clientHeight+Number.parseInt(e.style.bottom)>window.innerHeight&&(e.style.top="")}),e.addEventListener("mousedown",s),e.addEventListener("click",u),console.info(`LiveDebugger available at: ${o}`)});})();
