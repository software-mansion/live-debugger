defmodule LiveDebugger.Structs.LiveViewProcess do
  ...
  defstruct [:pid, :root?, :root_pid, :parent_pid, :socket_id]
  ...
  def new(pid, %Phoenix.LiveView.Socket{} = socket) when is_pid(pid) do
    new(pid, socket.root_pid, socket.parent_pid, socket.id)
  end

  def new(_, _), do: nil

  @spec new(pid(), pid(), pid() | nil, String.t()) :: t()
  def new(pid, root_pid, parent_pid, socket_id) do
    %__MODULE__{
      pid: pid,
      root?: root_pid == pid,
      root_pid: root_pid,
