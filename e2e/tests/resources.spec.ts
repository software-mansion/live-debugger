...
const resourcesTab = (page: Page) =>
  page.locator("a[href*='resources']").first();
...
const refreshSelectBtn = (page: Page) =>
  page.locator("button[aria-label='Refresh Rate']");
const refreshRadioBtn = (page: Page, value: number) =>
  page.locator(`input[type='radio'][value='${value}']`);

test('user can view process information in resources tab', async ({
  dbgApp,
}) => {
