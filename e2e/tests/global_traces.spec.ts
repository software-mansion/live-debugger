...
test.describe('Global Traces', () => {
  ...
  test('user can enable diff tracing and see diff traces', async ({
    ...
    const applyBtn = dbgApp.locator(
      '#filters-sidebar-form button[type="submit"]',
      { hasText: 'Apply' }
    );
    ...
    await expect(
      dbgApp.locator('pre', { hasText: '"diff"' }).first()
    ).toBeVisible();
  });
  ...
  test('user can go to specific node from global callbacks', async ({
    ...
    await dbgApp
      .locator('a', { hasText: 'LiveDebuggerDev.LiveViews.Main' })
      .first()
      .click();

    await expect(dbgApp.locator('text=LiveView')).toBeVisible();
    await expect(
      dbgApp.locator('text=LiveDebuggerDev.LiveViews.Main')
    ).toBeVisible();
  });
});
